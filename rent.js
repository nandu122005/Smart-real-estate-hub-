document.addEventListener("DOMContentLoaded", function () {
    ...
    // Retrieve the properties that are for rent
    const properties = JSON.parse(localStorage.getItem("properties")) || [];
    const propertiesForRent = properties.filter(property => property.type === "rent");

    // Display properties
    function displayProperties(propertiesForRent) {
        if (propertiesForRent.length === 0) {
            propertiesContainer.innerHTML = "<p>No properties for rent.</p>";
        } else {
            ...
            propertiesForRent.forEach(property => {
                ...
                propertyCard.innerHTML = `
                    ...
                    <button class="message-btn" data-property-id="${property.id}">Message Owner</button>
                `;
    ...
    sendMessageBtn.addEventListener("click", function () {
        ...
        const newMessage = {
            sender: loggedInUser,
            receiver: loggedInUser, // Replace with property owner email
            message: message,
    ...
    // Initial load of properties for rent
    displayProperties(propertiesForRent);
});
